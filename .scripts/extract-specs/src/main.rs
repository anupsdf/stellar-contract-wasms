...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() != 3 {
        eprintln!("Usage: {} <contracts_directory> <specs_directory>", args[0]);
        std::process::exit(1);
    }

    let contracts_dir = Path::new(&args[1]);
    let specs_dir = Path::new(&args[2]);
    ...
    for path in paths {
        ...
        match from_wasm(&wasm_bytes) {
            Ok(entries) => {
                ...
                let file_path = specs_dir.join(&*hash).with_extension("json");
                fs::write(&file_path, json)?;
                println!("Wrote specs for {}", hash);
            }
            Err(e) => {
                eprintln!("Failed to extract specs for {}: {}", hash, e);
            }
        }
    }
